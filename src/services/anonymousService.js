...
async function recordRecentPair(user1, user2, connection) {
    await connection.execute('INSERT INTO recent_pairs (user1, user2) VALUES (?, ?)', [user1, user2]);
}
...
async function startChat(userJid, sock, options = {}) {
    ...
    try {
        ...
        const [rows] = await connection.execute('SELECT partner_jid FROM anonymous_users WHERE jid = ?', [userJid]);
        if (rows.length > 0) {
        ...
        if (gender) {
            const oppositeGender = gender.toLowerCase() === 'male' || gender.toLowerCase() === 'pria' ? 'female' : 'male';
        ...
        if (candidate) {
            ...
            await connection.execute('INSERT INTO anonymous_users (jid, partner_jid, gender, topic) VALUES (?, ?, ?, ?)', [userJid, partnerJid, gender, topic]);
            // Perbarui kandidat agar memiliki pasangan
            await connection.execute('UPDATE anonymous_users SET partner_jid = ? WHERE jid = ?', [userJid, partnerJid]);
            ...
            await connection.execute('INSERT INTO anonymous_users (jid, partner_jid, gender, topic) VALUES (?, NULL, ?, ?)', [userJid, gender, topic]);
...
async function forwardMessage(sock, sender, msg) {
    const connection = await createConnection();
    try {
        const [rows] = await connection.execute('SELECT partner_jid FROM anonymous_users WHERE jid = ?', [sender]);
        if (rows.length > 0 && rows[0].partner_jid) {
            ...
            const quoted =
                msg.message.extendedTextMessage?.contextInfo?.quotedMessage || msg.message.extendedTextMessage?.contextInfo?.quotedMessage?.conversation;
...
async function exitChat(userJid, sock, options = {}) {
    ...
    try {
        ...
        await connection.execute('DELETE FROM anonymous_users WHERE jid = ?', [userJid]);
        ...
        if (partnerJid) {
            ...
            await connection.execute('UPDATE anonymous_users SET partner_jid = NULL WHERE jid = ?', [partnerJid]);
